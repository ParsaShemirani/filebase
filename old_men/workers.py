...
def get_directory_from_id(directory_id: str) -> Directory | None:
    with Session() as session:
        return session.scalar(select(Directory).where(Directory.id == directory_id))


def get_directory_file_from_id(directory_file_id: str) -> DirectoryFile | None:
    with Session() as session:
        return session.scalar(select(DirectoryFile).where(DirectoryFile.id == directory_file_id))
...
def insert_directory(name: str, parent_id: str | None) -> None:
    with Session() as session:
        with session.begin():
            directory = create_directory(name, parent_id)
            session.add(directory)

def insert_objects(object_list: list[object]) -> None:
    with Session() as session:
        with session.begin():
